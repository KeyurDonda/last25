...
class ScansController < ApplicationController
    ...
    def check_times
        url = params[:url]
        if Whitelist.find_by(url: url)
            if Scan.where(owner_id: current_user.id, url: url).count < 1
...
# Keyur Coded
    ...
    #active (A)
            ...
            @scanpageA=params.fetch(:active_page,1).to_i
            @currentPageNoA=@scanpageA
            @scanActCount = @counts[:active]['total'].to_f
            newCountA = (@scanActCount/10).to_i
            @newCountAct = (@scanActCount/10.0).to_f

        #lastpage logic

            if @scanActCount > 1
                if @newCountAct != newCountA
                    @newCountAct1 = newCountA+1
                else
                    @newCountAct1 = newCountA
                end
            else
                @currentPageNoA = newCountA+1
            end

        # nextpage logic

            if @scanActCount > 1
                if @scanpageA != @newCountAct1
                    @positiveA = @scanpageA+1
                else
                    @positiveA = @scanpageA
                end
            else
                @positiveA = newCountA+1
            end
    ...
    #finished (f)
            ...
            @scanpagef = params.fetch(:finished_page,1).to_i
            @currentPageNof=@scanpagef
            @scanFinCount = (@counts[:finished]['total']).to_f
    ...
    #suspended (s)
            ...
            @scanpages=params.fetch(:suspended_page,1).to_i
            @currentPageNos=@scanpages
            @scanSusCount = @counts[:suspended]['total'].to_f
            newCounts = (@scanSusCount/10).to_i
            @newCountSus = (@scanSusCount/10.0).to_f

        #lastpage logic

            if @scanSusCount > 1
                if @newCountSus != newCounts
                    @newCountSus1 = newCounts+1
                else
                    @newCountSus1 = newCounts
                end
            else
                @currentPageNos = newCounts+1
            end

        # nextpage logic

            if @scanSusCount > 1
                if @scanpages != @newCountSus1
                    @positives = @scanpages+1
                else
                    @positives = @scanpages
                end
            else
                @positives = newCounts+1
            end
...
# Keyur Coded End
    ...
    def send_email
        ...
        email = params[:email]
        UserMailer.welcome_email(file_url, email).deliver_now
